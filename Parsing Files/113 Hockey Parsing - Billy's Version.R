...
all_files <- list.files(path = "Output Folder/113 Hockey JSONs",
...
for (json_file_name in all_files){
  ...
  file_path <- paste0("Output Folder/113 Hockey JSONs/", json_file_name)
  ...
  Gender <- str_split(json_file_name, 
                      pattern = "'", 
                      n = 2)[[1]][1]
  ...
  # Write to CSV
  output_file_name <- paste("Output Folder/113 Hockey CSVs/", 
        # Only the actual date, not the time of game
        Gender, "-", Team1,"-", Team2, "-", substr(Date, 1,10), ".csv")
